...
export function RichRunInspector({ run, state, onClose }: { run: RunRecord; state: ControlState; onClose: () => void }) {
  ...
  return <aside className="inspector-panel rich-inspector">
    ...
    <div className="rich-run-summary"><StatusBadge status={currentRun.status} large /><h2>{String(currentRun.request.prompt ?? "Untitled run")}</h2><div className="run-summary-grid"><Detail label="Workspace" value="gravityclaw" /><Detail label="Trigger" value={String(currentRun.request.trigger ?? "Web")} /><Detail label="Started" value={formatTimestamp(currentRun.started_at ?? currentRun.created_at)} /><Detail label="Duration" value={runDuration(currentRun)} /><Detail label="Conversation" value={currentRun.conversation_id.slice(0, 12)} /><Detail label="Worker" value={currentRun.worker_id?.slice(0, 18) ?? "Not assigned"} /></div>{currentRun.status === "running" || currentRun.status === "queued" ? <button className="danger-button" onClick={() => void stop()}>■ Stop run</button> : <div className="run-terminal-note">Run {currentRun.status}. Historical state is preserved.</div>}{error && <div className="inline-error">{error}</div>}</div>
    <div className="inspector-tabs rich-tabs">{tabs.map((item) => <button key={item} className={tab === item ? "active" : ""} onClick={() => setTab(item)}>{item}</button>)}</div>
    <div className="rich-inspector-body">
      {tab === "Overview" && <RunOverview inspection={inspection} onTab={setTab} />}
      {tab === "Timeline" && <RunTimeline inspection={inspection} />}
      ...
      {tab === "Events" && <EventInspector events={inspection.events} raw={raw} onToggle={() => setRaw(!raw)} />}
    </div>
  </aside>;
}
...
function RunTimeline({ inspection }: { inspection: RunInspection }) { return <div className="run-timeline">{inspection.events.map((event) => <div className="run-timeline-row" key={event.id}><span className="timeline-time">{formatTimestamp(event.created_at, true)}</span><span className="timeline-icon">{eventIcon(event)}</span><div><strong>{eventLabel(event)}</strong><small>sequence {event.sequence}</small></div></div>)}{inspection.events.length === 0 && <EmptyState icon="◎" title="No events" detail="Persisted execution events will appear here." />}</div>; }
...
function EventInspector({ events, raw, onToggle }: { events: PersistedEvent[]; raw: boolean; onToggle: () => void }) { return <div className="event-inspector"><div className="event-view-toggle"><button className={!raw ? "active" : ""} onClick={onToggle}>Normalized</button><button className={raw ? "active" : ""} onClick={onToggle}>Raw</button></div>{events.map((event) => <details className="event-detail" key={event.id}><summary><span>{eventIcon(event)}</span><strong>#{event.id} {eventLabel(event)}</strong><small>{formatTimestamp(event.created_at, true)}</small></summary><pre>{JSON.stringify(raw ? event.raw ?? event.payload : event.payload, null, 2)}</pre></details>)}{events.length === 0 && <EmptyState icon="◎" title="No events" detail="The event stream is empty for this run." />}</div>; }
