...
export function RichRunInspector({ run, state, onClose }: { run: RunRecord; state: ControlState; onClose: () => void }) {
  ...
  return <aside className="inspector-panel rich-inspector">
    ...
    <div className="rich-run-summary"><StatusBadge status={currentRun.status} large /><h2>{String(currentRun.request.prompt ?? "Untitled run")}</h2><div className="run-summary-grid"><Detail label="Workspace" value="gravityclaw" /><Detail label="Trigger" value={String(currentRun.request.trigger ?? "Web")} /><Detail label="Started" value={formatTimestamp(currentRun.started_at ?? currentRun.created_at)} /><Detail label="Duration" value={runDuration(currentRun)} /><Detail label="Conversation" value={currentRun.conversation_id.slice(0, 12)} /><Detail label="Worker" value={currentRun.worker_id?.slice(0, 18) ?? "Not assigned"} /></div>{currentRun.status === "running" || currentRun.status === "queued" ? <button className="danger-button" onClick={() => void stop()}>■ Stop run</button> : <div className="run-terminal-note">Run {currentRun.status}. Historical state is preserved.</div>}{error && <div className="inline-error" role="alert">{error}</div>}</div>
