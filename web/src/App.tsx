  ...
  return <div className={`app ${theme}`}>
    ...
    <main className="main-shell">
      ...
      <div className="content-shell">
        ...
        {view === "conversations" && <ConversationWorkspace state={state} focus={focusMode} onToggleFocus={() => setFocusMode(!focusMode)} onRunSelect={setSelectedRun} />}
...
function ConversationWorkspace({ state, focus, onToggleFocus, onRunSelect }: { state: ControlState; focus: boolean; onToggleFocus: () => void; onRunSelect: (run: RunRecord) => void }) {
  ...
  const liveEvents = useMemo(() => [...timeline, ...state.activity.filter((event) => event.run_id === run?.id)]
    .filter((event, index, all) => all.findIndex((candidate) => candidate.id === event.id) === index), [timeline, state.activity, run?.id]);
  ...
  return <div className={`conversation-workspace ${focus ? "focus-mode" : "inspect-mode"}`}>
    ...
    <section className="conversation-main" aria-busy={loading}><div className="conversation-header"><div><div className="eyebrow">{detail?.conversation.channel ? `${detail.conversation.channel.toUpperCase()} · CONVERSATION` : "CONVERSATION"}</div><h1>{title}</h1></div><div className="conversation-header-actions"><button className={`mode-toggle ${focus ? "active" : ""}`} onClick={onToggleFocus} aria-pressed={focus}>{focus ? "Focus" : "Inspect"}</button>{run && <><StatusBadge status={presentation?.status ?? run.status} /><button className="secondary-button inspector-trigger" onClick={() => onRunSelect(run)}>Inspect run</button></>}</div></div>{error && <div className="inline-error workspace-error" role="alert">{error}</div>}{loading && !detail && <div className="workspace-loading" role="status" aria-live="polite">Loading conversation…</div>}{!detail && !loading && <EmptyState icon="◌" title="Choose a conversation" detail="Your durable conversations will appear here." />}{detail && <div className="message-scroll"><div className="message-list">{detail.messages.map((message) => <MessageCard key={message.id} message={message} />)}{run && presentation && (presentation.assistantText || presentation.currentTool || presentation.completedTools.length || presentation.subagents.length || run.status === "queued") && <LiveRunBlock run={run} presentation={presentation} />}</div></div>}<div className="composer-wrap"><div className="composer"><textarea aria-label="Message GravityClaw" value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void send(); } }} placeholder="Ask GravityClaw…" rows={1} disabled={!detail || sending} /><div className="composer-footer"><span className="composer-context">Workspace: <strong>gravityclaw</strong>{run?.status === "running" && <em> · follow-up will queue</em>}</span><button className="send-button" onClick={() => void send()} disabled={!draft.trim() || !detail || sending} aria-label={sending ? "Sending message" : "Send message"}>{sending ? "…" : "Send ↑"}</button></div></div></div></section>
